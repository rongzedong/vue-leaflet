...
function findClosestSegmentIndex(latlngs: any[], point: L.LatLng) {
  ...
  // Polygon为闭合图形，最后一个点和第一个点连成线段
  for (let i = 0; i < len; i++) {
    const a = latlngs[i];
    const b = latlngs[(i + 1) % len];
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    function onMarkerDblClick(idx: number) {
      const pts = points.value.slice();
      if (pts.length <= 3) return; // Polygon最少3点
      pts.splice(idx, 1);
      if (leafletObject.value) leafletObject.value.setLatLngs([pts]);
      context.emit("update:latLngs", pts);
    }
    ...
    onMounted(async () => {
      ...
      leafletObject.value = markRaw<L.Polygon>(
        polygon(points.value, options)
      );
      ...
      // 仅编辑模式绑定一次 polygon click
      leafletObject.value.on("click", onPolygonClick);
    ...
    watch(
      ...
      (edit) => {
        ...
        if (edit) {
          ...
          leafletObject.value.off("click", onPolygonClick);
        }
      }
    );
